...
class _MonthlyPrayerTimesScreenState extends State<MonthlyPrayerTimesScreen> {
  ...
  final _headers = ['اليوم', 'الفجر', 'الشروق', 'الظهر', 'العصر', 'المغرب', 'العشاء'];
        ...
        child: SafeArea(
          child: Column(
            children: [
              ...
              Expanded(child: _buildPrayerTable().animate().fadeIn(delay: 200.ms)),
              _buildActionButtons().animate().fadeIn(delay: 300.ms).slideY(begin: 0.1),
  ...
  Widget _buildHeader() {
    return Padding(
      ...
      child: Row(
        children: [
          IconButton(icon: const Icon(Icons.arrow_back, color: Color(0xFF333333)), onPressed: () => context.pop()),
          const Spacer(),
          const Text('مواقيت الصلاة الشهرية', style: TextStyle(color: Color(0xFF333333), fontWeight: FontWeight.bold, fontSize: 18)),
          const Spacer(),
        ...
        children: [
          GestureDetector(
            ...
            child: Container(
              ...
              child: const Icon(Icons.chevron_right, color: AppColors.primary, size: 22),
            ),
          ),
          Text('${_months[_currentMonth - 1]} $_currentYear', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16, color: Color(0xFF333333))),
          GestureDetector(
            onTap: () => _changeMonth(1),
            child: Container(
              padding: const EdgeInsets.all(6),
              decoration: BoxDecoration(color: AppColors.primary.withValues(alpha: 0.1), shape: BoxShape.circle),
              child: const Icon(Icons.chevron_left, color: AppColors.primary, size: 22),
            ),
          ),
        ],
      ),
          ...
          children: [
            ...
            Container(
              ...
              child: Row(
                children: _headers.map((h) => Expanded(
                  child: Text(h, textAlign: TextAlign.center, style: const TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: Color(0xFF757575))),
                )).toList(),
              ),
            ),
            // Table Body
            Expanded(
              child: ListView.builder(
                ...
                itemBuilder: (ctx, i) {
                  final row = data[i];
                  final isToday = _currentMonth == 5 && _currentYear == 2026 && (i + 1) == _todayDay;
                  return Container(
                    color: isToday ? const Color(0xFFE8F5E9) : (i.isEven ? Colors.white : const Color(0xFFFAFAFA)),
                    padding: const EdgeInsets.symmetric(vertical: 10, horizontal: 8),
                    child: Row(
                      children: [
        ...
        children: [
          SizedBox(
            ...
            child: ElevatedButton.icon(
              ...
              label: const Text('تحميل الجدول', style: TextStyle(fontWeight: FontWeight.bold)),
          ...
          TextButton.icon(
            ...
            label: const Text('إضافة إلى التقويم', style: TextStyle(color: AppColors.primary, fontWeight: FontWeight.w600)),
          ),
        ],
      ),
    );
